...
- [ ] 시작 시 LCD `CH1..CH8` / `0` 초기화 — 2026-05-01
  - 확인 항목: 노드 시작 직후 upper 라인에 `CH1..CH8`, lower 라인에 `0` 이 표시되는지
  - MCU LCD SysEx 초기화가 8채널 모두 적용되는지 실기 화면 확인
- [ ] 페이더 값 변경 시 LCD lower 갱신 — 2026-05-01
  - 확인 항목: 해당 채널 페이더 이동 시 lower 라인이 현재 값(`0..16383`)으로 즉시 갱신되는지
  - upper 라인 `CHn` 은 유지되고, 다른 채널 LCD 는 불필요하게 바뀌지 않는지 확인
